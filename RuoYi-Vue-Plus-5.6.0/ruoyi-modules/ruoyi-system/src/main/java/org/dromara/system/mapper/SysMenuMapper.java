...
public interface SysMenuMapper extends BaseMapperPlus<SysMenu, SysMenuVo> {
    ...
    /**
     * 根据菜单id列表查询权限（每用户自定义行走菜单用）
     *
     * @param menuIds 菜单id列表
     * @return 权限列表
     */
    default Set<String> selectMenuPermsByMenuIds(Collection<Long> menuIds) {
    ...
    /**
     * 根据权限码查询菜单中文名（权限提示用，无则返回 null）
     *
     * @param perms 权限码
     * @return 菜单名称
     */
    default String selectMenuNameByPerms(String perms) {
        if (StringUtils.isBlank(perms)) {
            return null;
        }
        SysMenu m = selectOne(new LambdaQueryWrapper<SysMenu>()
            .select(SysMenu::getMenuName)
            .eq(SysMenu::getPerms, perms)
            .last("limit 1"));
        return m == null ? null : m.getMenuName();
    }
