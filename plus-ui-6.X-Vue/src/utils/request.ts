...
// 权限提示去重：同一登录会话内"缺少功能权限"只提示一次
let permissionTipShown = false;
function showPermissionTipOnce(msg: string) {
  if (!permissionTipShown) {
    permissionTipShown = true;
    ElNotification.warning({ title: '缺少功能权限', message: msg, duration: 8000 });
  }
}
...
service.interceptors.response.use(
  ...
  async (error: any) => {
    ...
    if (httpStatus === 403 || String(error?.response?.data?.code) === '403') {
      showPermissionTipOnce(message);
    } else {
